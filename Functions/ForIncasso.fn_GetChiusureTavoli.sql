...
select * FROM [Accounting].[fn_GetStockLifeCycleInfo] (@gaming, 1) AS s
--*/
SELECT
	s.Tag,
	sc.CurrencyID,
...
FROM Accounting.fn_GetStockLifeCycleInfo (@gaming, 1) AS s
INNER JOIN 
(
...
(
	...
	AND ValueTypeID IN(1,36,42) --solo gettoni 
...
ORDER BY s.StockID,sc.CurrencyID
