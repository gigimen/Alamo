...
CREATE FUNCTION [GeneralPurpose].[fn_CancelLastTableResultsOnCISDisplay](@tablename [NVARCHAR](4000))
...
BEGIN
	...
	SELECT @ret = [GeneralPurpose].[asm_SendToCIS] (
	  'HOSTNAME="' + @table + '" EXC="CIS100" SRC="'+ @table + '" DST="ALL" TABLENAME="' + @table + '" GAMETYPE="' + left(@table,2) +'" TYPE="GC"',
	  @ip,--'10.41.41.8'
	  @port--,1000
	  )
	RETURN @ret
