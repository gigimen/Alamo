...
CREATE VIEW [Snoopy].[vw_AllAssegni]
WITH SCHEMABINDING
AS
SELECT 
		...
		a.Commissione,
		CASE WHEN cp.ContropartitaID IN (1,2) then 1 ELSE 0 end AS PrelievoEuro,
		a.CreditiGiocoRate,
		BA.BankName, 
		BA.AccountNr, 
		cu.CustomerID, 
		...
		(EMISS.Quantity * EMISS.Denomination ) / (1+a.Commissione) AS EuroNetti,
		(EMISS.Quantity * EMISS.Denomination ) * a.Commissione / (1+a.Commissione) AS CommissioneEuro,
		(EMISS.Quantity * EMISS.Denomination ) * a.Commissione / (1+a.Commissione) * EMISS.ExchangeRate AS CommissioneCHF,
		EMISS.ExchangeRate,
