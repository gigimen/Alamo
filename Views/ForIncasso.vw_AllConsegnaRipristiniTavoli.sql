...
CREATE VIEW [ForIncasso].[vw_AllConsegnaRipristiniTavoli]
WITH SCHEMABINDING
AS
SELECT TOP 100 PERCENT 
	...
	LF.LifeCycleID,
	LF.GamingDate,
	ISNULL(CON.ConsegnaEUR,0) AS ConsegnaEUR,
	ISNULL(RIP.RipristinoEUR,0) AS RipristinoEUR,
	ISNULL(CON.ConsegnaCHF,0) AS ConsegnaCHF,
	ISNULL(RIP.RipristinoCHF,0) AS RipristinoCHF
...
(
	SELECT 
		SourceLifeCycleID,
		TotalEURForSource AS ConsegnaEUR,
		TotalCHFForSource AS ConsegnaCHF 
...
(
	SELECT 
		SourceGamingDate,
		TotalCHFForSource AS RipristinoCHF,
		TotalEURForSource AS RipristinoEUR,
		DestStockID 
	FROM Accounting.vw_AllTransactionsEx 
