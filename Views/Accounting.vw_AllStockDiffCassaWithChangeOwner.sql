...
CREATE VIEW [Accounting].[vw_AllStockDiffCassaWithChangeOwner]
WITH SCHEMABINDING
AS
/*
	cc.Tag, 
	...
	SUM(cc.InitialQty * cc.Denomination ) AS InitialStock, 
        SUM(cc.Chiusura * cc.Denomination * cc.WeightInTotal) AS TotChiusura,
	SUM(cc.Consegna * cc.Denomination * cc.ERConsegna * cc.WeightInTotal) AS TotConsegna, 
        SUM(cc.Consegna * cc.Denomination * cc.ERConsegna * cc.WeightInTotal) + 
	SUM(cc.Chiusura * cc.Denomination* cc.WeightInTotal) - 
	SUM(cc.InitialQty * cc.Denomination* cc.WeightInTotal) AS DiffCassa
*/
SELECT 	
	ch.Tag, 
	ch.StockID,
	...
	chn.OwnerName AS ChnOwner,
	chn.OwnerUserID AS ChnOwnerUserID, 
	chn.SnapshotTimeLoc AS ChngOwnerTimeLoc,
	chn.TotChngOwner,
	chn.TotChngOwner - ch.InitialStock AS DiffCassaChnOwner
FROM   
(
SELECT 
	sto.Tag, 
...
SELECT 
	...
	UA.UserID AS OwnerUserID, 
	US.LastName + ' ' + US.FirstName AS OwnerName, 
	SUM(chV.Quantity * chV.ExchangeRate * deno.Denomination * scd.WeightInTotal) AS TotChngOwner
FROM Accounting.tbl_Snapshots ch 
	INNER JOIN Accounting.tbl_LifeCycles lf ON ch.LifeCycleID = lf.LifeCycleID 
	INNER JOIN CasinoLayout.Stocks sto ON lf.StockID = sto.StockID 
	INNER JOIN CasinoLayout.StockComposition_Denominations scd ON scd.StockCompositionID = lf.StockCompositionID
...
GROUP BY sto.Tag, 
	...
	US.LastName,
	US.FirstName
) chn ON chn.LifeCycleID = ch.LifeCycleID
LEFT OUTER JOIN
(
SELECT 
	lf.LifeCycleID,
