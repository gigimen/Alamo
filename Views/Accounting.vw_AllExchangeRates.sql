...
CREATE VIEW [Accounting].[vw_AllExchangeRates]
WITH SCHEMABINDING
AS
SELECT  
	today.GamingDate, 
...
INNER JOIN CasinoLayout.tbl_Currencies cu ON today.CurrencyID = cu.CurrencyID
INNER JOIN CasinoLayout.tbl_ValueTypes vt ON cu.CurrencyID = vt.CurrencyID
...
WHERE vt.ValueTypeID  <= 28 --avoid new valutypes in euros
